...
public class pb_Stl_Tests {
    ...
    public void TestExportMultiple() {
        ...
        IList<Mesh> expected = pb_Stl_Importer.Import(string.Format("{0}/CompositeCubes_BINARY.stl", TEST_MODELS));
        IList<Mesh> results = pb_Stl_Importer.Import(temp_model_path);
    ...
    public void TestImportAscii() {
        IList<Mesh> meshes = pb_Stl_Importer.Import(string.Format("{0}/Cylinder_ASCII_RH.stl", TEST_MODELS));
    ...
    public void TestImportBinary() {
        IList<Mesh> meshes = pb_Stl_Importer.Import(string.Format("{0}/Cylinder_BINARY_RH.stl", TEST_MODELS));
        Assert.IsTrue(meshes != null);
        Assert.AreEqual(1, meshes.Count);
        Assert.AreEqual(240, meshes[0].triangles.Length);
        Assert.AreEqual(240, meshes[0].vertexCount);
    ...
    public void TestImportBinaryWithHeaders() {
        IList<Mesh> meshes = pb_Stl_Importer.Import(string.Format("{0}/CubedShape_BINARY_H.stl", TEST_MODELS));
        Assert.IsTrue(meshes != null);
        Assert.AreEqual(1, meshes.Count);
